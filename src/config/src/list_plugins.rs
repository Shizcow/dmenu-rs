...
fn main() {
    ...
    table_head.add_row(Row::new(vec![Cell::new("Available Plugins")
    ...
    table.set_titles(Row::new(vec![
        Cell::new_align("Sel", Alignment::CENTER).with_style(Attr::Bold),
        Cell::new_align("Name", Alignment::CENTER).with_style(Attr::Bold),
        Cell::new_align("Description", Alignment::CENTER).with_style(Attr::Bold),
    ]));
    ...
    for entry in glob(&plugin_path.display().to_string()).expect("Failed to read glob pattern") {
        match entry {
            Err(e) => println!("{:?}", e),
            Ok(path) => {
                ...
                table.add_row(Row::new(vec![
                    Cell::new_align(
                        if selected_plugins.contains(&plugin_name) {
                            "X"
                        } else {
                            " "
                        },
                    ...
                    Cell::new(about).with_style(Attr::Italic(true)),
                ]));
            }
        }
    }
    ...
    table_footer.add_row(Row::new(vec![Cell::new(
        "To enable plugins, read the PLUGINS section in config.mk",
    )
    .with_style(Attr::Bold)
    .with_style(Attr::ForegroundColor(color::BLUE))]));
