...
fn main() {
    ...
    for plugin in plugins {
        ...
        watch_globs.push((
            format!(
                "../plugins/{}/{}",
                plugin,
                get_yaml_top_level(&mut plugin_yaml, "entry")
                    .expect("No args found in yaml object")
            ), //relative to other build script
    ...
    if let Err(err) = deps_finished_file.write_all(deps_vec.join("\n").as_bytes()) {
        panic!(
            "Could not write generated dependency file to OUT_DIR: {}",
            err
        );
    }
    ...
    manpage
        ...
        .build(
            "This dmenu is dmenu-rs, a rewrite of dmenu in rust. It's faster and more \
		 flexible.",
        );
    ...
    for arg in yaml_args {
        ...
        match hash.get(&keys[0]) {
            Some(Yaml::Hash(hash)) => {
                ...
                for key in &keys {
                    ...
                    let keyvalue = match hash.get(key) {
                        Some(Yaml::String(string)) => string,
                        _ => continue,
                    };
        ...
        if short.is_some() || long.is_some() {
            manpage.arg(
                short.map(|s| s.chars().nth(0).unwrap()),
                long.map(|s| s.to_string()),
                inputs,
                help.expect("yaml: help must be provided").to_string(),
            );
        }
    }
    ...
    let watch_indicator_string = watch_globs
        .into_iter()
        .map(|(glob, alias)| format!("{}\n{}\n", glob, alias))
        .join("\n");
