...
fn main() {
    ...
    usebuilder.write_to_file_all(out_path.join("proc_mod_plugin.rs"));
    ...
    for plugin in plugins {
        let mut plugin_yaml =
            util::get_yaml(&format!("../plugins/{}/plugin.yml", plugin), Some(&plugin));
        println!(
            "cargo:rerun-if-changed=../plugins/{}/{}",
            plugin,
            util::get_yaml_top_level(&mut plugin_yaml, "entry").unwrap()
        );
    }
